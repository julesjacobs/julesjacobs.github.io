...
There **is** a *good* non-numerical reason to talk about entropy and log probabilities, and that is that they are additive rather than multiplicative. If you have two independent probabilities $$p$$ and $$q$$, then the probability of both of them happening is $$p \cdot q$$, but the log probability of both of them happening is $$\log p + \log q$$. In other words, log probabilities let you combine independent effects with addition. We like addition more than multiplication.
