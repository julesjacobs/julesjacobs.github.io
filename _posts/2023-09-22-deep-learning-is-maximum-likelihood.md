...
We can view $$f_\theta$$ as a generative model: given some features $$x$$, we generate a label $$y$$ according to the probability distribution $$f_\theta(x)_y$$. Like how ChatGPT generates the next word. Maximum likelihood says that we should choose the parameters $$\theta$$ that make the training examples most likely to be generated. Makes sense, right? Rather than choosing some $$\theta$$ that makes $f_\theta$$ generate some totally random output, we should choose $$\theta$$ that makes $$f_\theta$$ generate the training input-output examples.
