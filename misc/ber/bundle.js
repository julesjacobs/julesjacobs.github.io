  ...
  Cons ((fun x -> Cons(x, Nil)), Cons (bool_not, Nil))`;
    ...
    var renderMarkedText = (text, marks2, cls) => {
      ...
      for (const m of sorted) {
        ...
        if (start > cursor2) {
          html += escapeHtml(text.slice(cursor2, start));
        }
    ...
    var typecheckDomHandlers = EditorView.domEventHandlers({
      keydown: (event) => {
    ...
    var spanToRange = (span) => {
      const doc2 = view.state.doc;
      const clampLine = (line) => Math.max(1, Math.min(doc2.lines, line));
      const startLine = clampLine(span.startLine);
      const endLine = clampLine(span.endLine);
      const startLineInfo = doc2.line(startLine);
      const endLineInfo = doc2.line(endLine);
