...
\begin{abstract}
  \noindent
  We will investigate these three well-known techniques for rewriting to normal form, and how
  to use them to optimize regular expressions and compute $\beta$ normal forms of lambda terms.
...
\subsection{Folding HOAS \& finally tagless style}

The \id{toTm} : \id{Hs} $\to$ \id{Tm} and \id{norm} : \id{Hs} $\to$ \id{Hs} functions are somewhat similar.
We can factor out the pattern by defining a \id{fold} function on \id{Hs}, using which we can write functions \id{Hs} $\to$ \id{T} for any \id{T}.
The \id{fold} function takes smart constructors \id{app} : \id{T} $\times$ \id{T} $\to$ \id{T} and \id{lam} : (\id{T} $\to$ \id{T}) $\to$ \id{T} as arguments, and gives \id{fold(a:Hs, app,lam)} : \id{T}.
...
We can now rewrite \id{toTm} and \id{norm} as one-liners:
...
We can also represent \id{a:Hs} values \emph{as their folding function} \id{(app,lam) => fold(a, app, lam)}.
...
In $\eta$ expanded form we can only use $f : A \to B$ as the first argument of $\ap$ because $f$ has a function type;
if we want to use $f$ in any other place then we must instead write $\lambda x. \ap(f,x)$.
...
Although the \id{Sem} domain doesn't have a case for \id{App}, we can still write a smart constructor for it:
...
Since we assume that \id{a} is of function type, we can ignore the \id{Syn} case.
