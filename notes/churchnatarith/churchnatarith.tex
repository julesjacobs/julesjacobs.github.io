...
The notation makes the proofs that this does arithmetic correctly a triviality: if $[n]$ is the Church numeral corresponding to an ordinary natural number $n \in \N$ (\ie satisfying $f^{[n]} = f^n$, where $f^{[m]} \equiv [m](f)$ according to our notation, and $f^n$ for ordinary natural number $n \in \N$ is $n$-fold function composition), then
\begin{align*}
