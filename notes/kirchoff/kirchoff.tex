...
\begin{definition}
  ...
  Compare this with the sign of a permutation graph, for which a cycle with an odd number of edges does not affect the sign, and a cycle with an even number of edges flips the sign. The sign of a TC-labeled 1-graph can thus determined by the same method, except that we consider only C-edges.
