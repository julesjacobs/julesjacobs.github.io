...
Given a function $f : \{0,1\}^k \to \{0,1\}^k$ on bit strings of length $k$, we define a classical gate $\Cl_f$ given by:
\begin{align}
...
\begin{align*}
  ...
  &= \Ha^{\otimes n+1} \cdot \Cl_f \frac{1}{\sqrt{2}^{n+1}} \sum_x (\ket{x0} - \ket{x1}) \nexttag{perform $\Cl_f$}\\
  &= \Ha^{\otimes n+1} \frac{1}{\sqrt{2}^{n+1}} \sum_x (\ket{x(0 \oplus g(x))} - \ket{x(1 \oplus g(x))}) \nexttag{rewrite}\\
  &= \Ha^{\otimes n+1} \frac{1}{\sqrt{2}^{n+1}} \sum_x (-1)^{g(x)}(\ket{x0} - \ket{x1}) \nexttag{perform $\Ha$ on the last bit}\\
