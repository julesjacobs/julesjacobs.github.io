...
The Deutsch–Jozsa algorithm \cite{DeutschJozsaAlgorithm2021} for this problem operates on $n+1$ bits and is given by
\begin{align*}
...
and will give a different output if $g$ is balanced.
...
Let us first see what the operator $H^{\otimes n}$ does on a general definite state $\ket{x}$, where $x$ is a bit string of length $n$.
...
Notice that the terms sum to $1$ if $g$ is constant.
