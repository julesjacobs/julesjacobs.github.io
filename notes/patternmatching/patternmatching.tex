...
\begin{alignat*}{4}
  ...
  \clause{\ba \is \fvar x}{e_7}
\end{alignat*}
Notice how we now have multiple equations per clause, corresponding to the $\fvar{a_1}$ and $\fvar{a_2}$ in the generated $\mathbf{match}$, which become bound variables $\bvar{a_1}$ and $\bvar{a_2}$ in $A$.
