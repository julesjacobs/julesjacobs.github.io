...
We can simplify these problems by pushing bare variable tests such as $\bvar{a_2} \is \fvar y$ into the right hand sides. Then the sub problem for $A$ becomes:
\begin{alignat*}{4}
  [A]\ =\\
  \matchclauses \\
  \clause{\bvar{a_1} \is \Zero, \bvar{a_2} \is \Zero}{e_1} \\
...
\begin{enumerate}
  \item Push bare variable tests $\ba \is \fvar y$ into the right hand sides, so that all the remaining tests test constructors.
