...
\begin{tabular}{c l}
  ...
  Any of the above & \tac{[]}\quad (unpack with names chosen by Coq) \\
  $x = y$ & \tac{->} or \tac{<-} \\
...
Because $\exists x, P$, $P \land Q$, $P \lor Q$, $False$ are \emph{defined} as inductive types, their intro patterns are special cases of the intro pattern for inductive types.
